...
{
    if ((SX126xGetOperatingMode( ) == MODE_SLEEP) || (SX126xGetOperatingMode( ) == MODE_RX_DC) || (SX126xGetOperatingMode( ) == MODE_RX))
    {
        SX126xWakeup( );
    ...
    else if ((SX126xGetOperatingMode( ) == MODE_TX) || (SX126xGetOperatingMode( ) == MODE_STDBY_RC))
    {
        SX126xAntSwOn( );
    }
    ...
    if (SX126xGetOperatingMode( ) == MODE_TX)
    {
        if (joined_ledend == 0)
        {
            gpio_init(LED_RGB_PORT, LED_BLUE_PIN, GPIO_MODE_OUTPUT_PP_HIGH);
        }
    }
    else
    {

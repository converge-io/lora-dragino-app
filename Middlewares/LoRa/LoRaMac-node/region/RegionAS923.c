...
static bool VerifyTxFreq(uint32_t freq)
{
    ...
    if ((freq < 920600000) || (freq > 923400000))
    {
        return false;
        // Japan region channels used are 24~38 (Center Freq: 920.6 MHz to 923.4 MHz @ 200 kHz max BW each)
        // Note: We do not use Japan channel plan for 33~61 due to unavailability of channel-by-channel controls for LBT, TX time, Pause time and Duty cycle in this stack.
    }
        #else
...
bool RegionAS923TxConfig(TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir)
{
    ...
    phyTxPower = RegionCommonComputeTxPower(txPowerLimited, AS923_DEFAULT_MAX_EIRP, 0);
...
uint8_t RegionAS923RxParamSetupReq(RxParamSetupReqParams_t* rxParamSetupReq)
{
    ...
    if (VerifyTxFreq(rxParamSetupReq->Frequency) == false)
    {
...
//            {
//
//                Radio.SetFskBandwidth( AS923_JAPAN_CHANNEL_MAX_BANDWIDTH );
//                rssi = Radio.GetChannelRssi( MODEM_FSK, Channels[channelNext].Frequency);
//
//                if( rssi > AS923_RSSI_FREE_TH )
//                {
//                    //DBG_PRINTF("\tChannel busy: %d Hz @ %d dBm\n\r", Channels[channelNext].Frequency, rssi);
//                    channelFree = false;
//                    break;
//                }
//				    }
...
//            {
//                // Free channel found
//                //DBG_PRINTF("\tChannel FREE: %d Hz @ %d dBm\n\r", Channels[channelNext].Frequency, rssi);
//                *channel = channelNext;
//                *time = 0;
//                return true;
//            }
            if (Radio.IsChannelFree(MODEM_FSK, Channels[channelNext].Frequency, AS923_RSSI_FREE_TH, AS923_CARRIER_SENSE_TIME) == true)
            {
                // Free channel found
                *channel = channelNext;
                *time = 0;

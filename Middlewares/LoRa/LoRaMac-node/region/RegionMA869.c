 ...
 * \file      RegionMA869.c
 *
 * \brief     Region implementation for MA869
 *
 ...
 * LoRaMAC channels
 */
static ChannelParams_t Channels[MA869_MAX_NB_CHANNELS];

/*!
 * LoRaMac bands
 */
static Band_t Bands[MA869_MAX_NB_BANDS] =
{
    MA869_BAND0,
    MA869_BAND1,
    MA869_BAND2,
...
{
    switch (BandwidthsMA869[drIndex])
    {
        default:
        case 125000:
            return 0;
...
static uint8_t CountNbOfEnabledChannels(bool joined, uint8_t datarate, uint16_t* channelsMask, ChannelParams_t* channels, Band_t* bands, uint8_t* enabledChannels, uint8_t* delayTx)
{
    ...
    for ( uint8_t i = 0, k = 0; i < MA869_MAX_NB_CHANNELS; i += 16, k++ )
    {
...
PhyParam_t RegionMA869GetPhyParam(GetPhyParams_t* getPhy)
{
    ...
    switch (getPhy->Attribute)
    {
        case PHY_MIN_RX_DR:
        {
            phyParam.Value = MA869_RX_MIN_DATARATE;
            break;
        }
        case PHY_MIN_TX_DR:
        {
            phyParam.Value = MA869_TX_MIN_DATARATE;
            break;
        }
        case PHY_DEF_TX_DR:
        {
            phyParam.Value = MA869_DEFAULT_DATARATE;
            break;
        }
        case PHY_NEXT_LOWER_TX_DR:
        {
            phyParam.Value = GetNextLowerTxDr(getPhy->Datarate, MA869_TX_MIN_DATARATE);
            break;
        }
        case PHY_DEF_TX_POWER:
        {
            phyParam.Value = MA869_DEFAULT_TX_POWER;
            break;
        }
        case PHY_MAX_PAYLOAD:
        {
            phyParam.Value = MaxPayloadOfDatarateMA869[getPhy->Datarate];
            break;
        }
        case PHY_MAX_PAYLOAD_REPEATER:
        {
            phyParam.Value = MaxPayloadOfDatarateRepeaterMA869[getPhy->Datarate];
            break;
        }
        case PHY_DUTY_CYCLE:
        {
            phyParam.Value = MA869_DUTY_CYCLE_ENABLED;
            break;
        }
        case PHY_MAX_RX_WINDOW:
        {
            phyParam.Value = MA869_MAX_RX_WINDOW;
            break;
        }
        case PHY_RECEIVE_DELAY1:
        {
            phyParam.Value = MA869_RECEIVE_DELAY1;
            break;
        }
        case PHY_RECEIVE_DELAY2:
        {
            phyParam.Value = MA869_RECEIVE_DELAY2;
            break;
        }
        case PHY_JOIN_ACCEPT_DELAY1:
        {
            phyParam.Value = MA869_JOIN_ACCEPT_DELAY1;
            break;
        }
        case PHY_JOIN_ACCEPT_DELAY2:
        {
            phyParam.Value = MA869_JOIN_ACCEPT_DELAY2;
            break;
        }
        case PHY_MAX_FCNT_GAP:
        {
            phyParam.Value = MA869_MAX_FCNT_GAP;
            break;
        }
        case PHY_ACK_TIMEOUT:
        {
            phyParam.Value = (MA869_ACKTIMEOUT + randr(-MA869_ACK_TIMEOUT_RND, MA869_ACK_TIMEOUT_RND));
            break;
        }
        case PHY_DEF_DR1_OFFSET:
        {
            phyParam.Value = MA869_DEFAULT_RX1_DR_OFFSET;
            break;
        }
        case PHY_DEF_RX2_FREQUENCY:
        {
            phyParam.Value = MA869_RX_WND_2_FREQ;
            break;
        }
        case PHY_DEF_RX2_DR:
        {
            phyParam.Value = MA869_RX_WND_2_DR;
            break;
        }
        ...
        {
            phyParam.Value = MA869_MAX_NB_CHANNELS;
            break;
        }
        case PHY_CHANNELS:
        {
        ...
        {
            phyParam.fValue = MA869_DEFAULT_MAX_EIRP;
            break;
        }
        case PHY_DEF_ANTENNA_GAIN:
        {
            phyParam.fValue = MA869_DEFAULT_ANTENNA_GAIN;
            break;
        }
...
void RegionMA869InitDefaults(InitType_t type)
{
    ...
    switch (type)
    {
        case INIT_TYPE_INIT:
        {
            ...
            {
                // Channels
                Channels[0] = (ChannelParams_t)MA869_LC1;
                Channels[1] = (ChannelParams_t)MA869_LC2;
                Channels[2] = (ChannelParams_t)MA869_LC3;
...
bool RegionMA869Verify(VerifyParams_t* verify, PhyAttribute_t phyAttribute)
{
    switch (phyAttribute)
    {
        case PHY_TX_DR:
        {
            return RegionCommonValueInRange(verify->DatarateParams.Datarate, MA869_TX_MIN_DATARATE, MA869_TX_MAX_DATARATE);
        }
        case PHY_DEF_TX_DR:
        {
        ...
        case PHY_RX_DR:
        {
            return RegionCommonValueInRange(verify->DatarateParams.Datarate, MA869_RX_MIN_DATARATE, MA869_RX_MAX_DATARATE);
        }
        case PHY_DEF_TX_POWER:
        case PHY_TX_POWER:
        {
            // Remark: switched min and max!
            return RegionCommonValueInRange(verify->TxPower, MA869_MAX_TX_POWER, MA869_MIN_TX_POWER);
        }
        case PHY_DUTY_CYCLE:
        {
            return MA869_DUTY_CYCLE_ENABLED;
        }
        case PHY_NB_JOIN_TRIALS:
        {
...
void RegionMA869ApplyCFList(ApplyCFListParams_t* applyCFList)
{
    ...
    for ( uint8_t i = 0, chanIdx = MA869_NUMB_DEFAULT_CHANNELS; chanIdx < MA869_MAX_NB_CHANNELS; i += 3, chanIdx++ )
    {
        if (chanIdx < (MA869_NUMB_CHANNELS_CF_LIST + MA869_NUMB_DEFAULT_CHANNELS))
        {
            // Channel frequency
        ...
        if (newChannel.Frequency != 0)
        {
            ...
            RegionMA869ChannelAdd(&channelAdd);
        }
        else
        {
            channelRemove.ChannelId = chanIdx;

            RegionMA869ChannelsRemove(&channelRemove);
        }
    }
}
...
bool RegionMA869AdrNext(AdrNextParams_t* adrNext, int8_t* drOut, int8_t* txPowOut, uint32_t* adrAckCounter)
{
    ...
    *adrAckCounter = adrNext->AdrAckCounter;
    ...
    {
        if (datarate == MA869_TX_MIN_DATARATE)
        {
            *adrAckCounter = 0;
            adrAckReq = false;
        }
        ...
        {
            if (adrNext->AdrAckCounter >= MA869_ADR_ACK_LIMIT)
            {
                adrAckReq = true;
                txPower = MA869_MAX_TX_POWER;
            }
            else
            {
                adrAckReq = false;
            }
            if (adrNext->AdrAckCounter >= (MA869_ADR_ACK_LIMIT + MA869_ADR_ACK_DELAY))
            {
                if ((adrNext->AdrAckCounter % MA869_ADR_ACK_DELAY) == 1)
                {
                    // Decrease the datarate
                    getPhy.Attribute = PHY_NEXT_LOWER_TX_DR;
                    getPhy.Datarate = datarate;
                    getPhy.UplinkDwellTime = adrNext->UplinkDwellTime;
                    phyParam = RegionMA869GetPhyParam(&getPhy);
...
void RegionMA869ComputeRxWindowParameters(int8_t datarate, uint8_t minRxSymbols, uint32_t rxError, RxConfigParams_t* rxConfigParams)
{
    ...
    rxConfigParams->Datarate = MIN(datarate, MA869_RX_MAX_DATARATE);
    rxConfigParams->Bandwidth = GetBandwidth(rxConfigParams->Datarate);

    if (rxConfigParams->Datarate == DR_7)
    { // FSK
        tSymbol = RegionCommonComputeSymbolTimeFsk(DataratesMA869[rxConfigParams->Datarate]);
    }
    else
    { // LoRa
        tSymbol = RegionCommonComputeSymbolTimeLoRa(DataratesMA869[rxConfigParams->Datarate], BandwidthsMA869[rxConfigParams->Datarate]);
    }
...
bool RegionMA869RxConfig(RxConfigParams_t* rxConfig, int8_t* datarate)
{
    ...
    {
        maxPayload = MaxPayloadOfDatarateRepeaterMA869[dr];
    }
    else
    {
        maxPayload = MaxPayloadOfDatarateMA869[dr];
    }
    ...
    if (debug_flags == 1)
    {
        ...
        LOG_PRINTF(LL_DEBUG, "[%lu]", (long unsigned int)ts);
    ...
    else
    {
        ...
        LOG_PRINTF(LL_DEBUG, "RX on freq %d.%d MHz at DR %d\r\n",rxfreq_a,rxfreq_b,dr);
    }
...
{
    RadioModems_t modem;
    int8_t phyDr = DataratesMA869[txConfig->Datarate];
    ...
    else
    {
        ...
        LOG_PRINTF(LL_DEBUG, "TX on freq %d.%d MHz at DR %d\r\n",txfreq_a,txfreq_b,txConfig->Datarate);
    }
...
uint8_t RegionMA869LinkAdrReq(LinkAdrReqParams_t* linkAdrReq, int8_t* drOut, int8_t* txPowOut, uint8_t* nbRepOut, uint8_t* nbBytesParsed)
{
    ...
    while (bytesProcessed < linkAdrReq->PayloadSize)
    {
        ...
        {
            for ( uint8_t i = 0; i < MA869_MAX_NB_CHANNELS; i++ )
            {
    ...
    linkAdrVerifyParams.NbChannels = MA869_MAX_NB_CHANNELS;
    linkAdrVerifyParams.ChannelsMask = &chMask;
    linkAdrVerifyParams.MinDatarate = (int8_t)phyParam.Value;
    linkAdrVerifyParams.MaxDatarate = MA869_TX_MAX_DATARATE;
    linkAdrVerifyParams.Channels = Channels;
    linkAdrVerifyParams.MinTxPower = MA869_MIN_TX_POWER;
    linkAdrVerifyParams.MaxTxPower = MA869_MAX_TX_POWER;
...
uint8_t RegionMA869RxParamSetupReq(RxParamSetupReqParams_t* rxParamSetupReq)
{
    ...
    if (VerifyTxFreq(rxParamSetupReq->Frequency, &band) == false)
    {
        status &= 0xFE; // Channel frequency KO
    }

    // Verify datarate
    if (RegionCommonValueInRange(rxParamSetupReq->Datarate, MA869_RX_MIN_DATARATE, MA869_RX_MAX_DATARATE) == false)
    {
        status &= 0xFD; // Datarate KO
    }

    // Verify datarate offset
    if (RegionCommonValueInRange(rxParamSetupReq->DrOffset, MA869_MIN_RX1_DR_OFFSET, MA869_MAX_RX1_DR_OFFSET) == false)
    {
...
uint8_t RegionMA869NewChannelReq(NewChannelReqParams_t* newChannelReq)
{
    ...
    if (newChannelReq->NewChannel->Frequency == 0)
    {
        ...
        // Remove
        if (RegionMA869ChannelsRemove(&channelRemove) == false)
        {
            status &= 0xFC;
        }
    }
    else
    {
        ...
        switch (RegionMA869ChannelAdd(&channelAdd))
        {
...
bool RegionMA869NextChannel(NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff)
{
    uint8_t nbEnabledChannels = 0;
    uint8_t delayTx = 0;
    uint8_t enabledChannels[MA869_MAX_NB_CHANNELS] = { 0 };
    ...
    if (nextChanParams->AggrTimeOff <= TimerGetElapsedTime(nextChanParams->LastAggrTx))
    {
        ...
        *aggregatedTimeOff = 0;
        ...
        nextTxDelay = RegionCommonUpdateBandTimeOff(nextChanParams->Joined, nextChanParams->DutyCycleEnabled, Bands, MA869_MAX_NB_BANDS);
...
LoRaMacStatus_t RegionMA869ChannelAdd(ChannelAddParams_t* channelAdd)
{
    ...
    if (id < MA869_NUMB_DEFAULT_CHANNELS)
    {
        return LORAMAC_STATUS_FREQ_AND_DR_INVALID;
    }

    if (id >= MA869_MAX_NB_CHANNELS)
    {
        return LORAMAC_STATUS_PARAMETER_INVALID;
    }

    // Validate the datarate range
    if (RegionCommonValueInRange(channelAdd->NewChannel->DrRange.Fields.Min, MA869_TX_MIN_DATARATE, MA869_TX_MAX_DATARATE) == false)
    {
        drInvalid = true;
    }
    if (RegionCommonValueInRange(channelAdd->NewChannel->DrRange.Fields.Max, MA869_TX_MIN_DATARATE, MA869_TX_MAX_DATARATE) == false)
    {
        drInvalid = true;
    }
...
//        if( RegionCommonValueInRange( channelAdd->NewChannel->DrRange.Fields.Max, DR_5, MA869_TX_MAX_DATARATE ) == false )
//        {
//            drInvalid = true;
//        }
...
bool RegionMA869ChannelsRemove(ChannelRemoveParams_t* channelRemove)
{
    ...
    return RegionCommonChanDisable(ChannelsMask, id, MA869_MAX_NB_CHANNELS);
}
...
//    regionCommonRxBeaconSetup.Datarates = DataratesMA869;
//    regionCommonRxBeaconSetup.Frequency = rxBeaconSetup->Frequency;
//    regionCommonRxBeaconSetup.BeaconSize = MA869_BEACON_SIZE;
//    regionCommonRxBeaconSetup.BeaconDatarate = MA869_BEACON_CHANNEL_DR;
//    regionCommonRxBeaconSetup.BeaconChannelBW = MA869_BEACON_CHANNEL_BW;

...
static uint32_t calc_uart_baud(uint32_t uart_clk, uint32_t baud)
{
    uint32_t int_div;
    uint32_t fac_div;
    uint32_t remainder;
 ...
 * @note  The interrupt UART_INTERRUPT_RX_DONE will be tiggered when the receive fifo is not less than the fifo level.
 *        The default value is UART_RX_FIFO_LEVEL_1_2.
 * @param uartx Select the UART peripheral number(UART0, UART1, UART2 and UART3)
 ...
 * @note  The interrupt UART_INTERRUPT_TX_DONE will be tiggered when the transmit fifo is not greater than the fifo level.
 *        The default value is UART_TX_FIFO_LEVEL_1_2.
 * @param uartx Select the UART peripheral number(UART0, UART1, UART2 and UART3)
 * @param uart_fifo_level The threshold
